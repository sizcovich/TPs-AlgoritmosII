...
\begin{algorithmic}[1]
	\State $res.it \gets crearIt(l)$\Comment{$\Theta(1)$}
\end{algorithmic}
...
\begin{algorithmic}[1]
	\State $res \gets haySiguiente(iterador.it)$ \Comment{$\Theta(1)$}
\end{algorithmic}
	\textbf{C\'alculo de complejidad}: $\Theta(1)$
\end{algorithm}

\begin{algorithm}[H]
\caption*{iSiguiente(\Inout{iterador}{iteradorUni($\alpha$)}) $\DRef res$ : \TipoVariable{$\alpha$}}
\begin{algorithmic}[1]
	\State Avanzar($iterador.it$)\Comment{$\Theta(1)$}
\end{algorithmic}
	\textbf{C\'alculo de complejidad}: $\Theta(1)$
\end{algorithm}
...
\begin{algorithmic}[1]
	\State siguiente(iterador.it)\Comment{$\Theta(1)$}
\end{algorithmic}
	\textbf{C\'alculo de complejidad}: $\Theta(1)$
\end{algorithm}
