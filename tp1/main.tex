...
\tadExporta{pila($\alpha$), generadores, observadores, tama\~no}
\tadUsa{\tadNombre{Bool, Nat}}
...
\tadOtrasOperaciones
\tadOperacion{tama\~no}{pila($\alpha$)}{nat}{}
...
\tadAxioma{tama\~no($p$)}{\IF vacía?($p$) THEN 0 ELSE 1 + tama\~no(desapilar($p$)) FI}
...
\tadIgualdadObservacional{c}{c'}{cola($\alpha$)}{vac\'ia?($c$) $\igobs$ vac\'ia?($c'$) $\land$ \\ 
                                                 ($\neg$vac\'ia?($c$) $\implies$ (pr\'oximo($c$) $\igobs$ pr\'oximo($c'$) $\land$ \\
                                        \phantom{($\neg$vac\'ia?($c$) $\implies$ (}desencolar($c$) $\igobs$ desencolar($c'$))}
...
\tadExporta{cola($\alpha$), generadores, observadores, tama\~no}
\tadUsa{\tadNombre{Bool, Nat}}

\tadAlinearFunciones{desencolar}{$\alpha$,cola($\alpha$)}

\tadObservadores
\tadOperacion{vac\'ia?}{cola($\alpha$)}{bool}{}
\tadOperacion{pr\'oximo}{cola($\alpha$)/c}{$\alpha$}{$\neg$ vac\'ia?($c$)}
...
\tadOtrasOperaciones
\tadOperacion{tama\~no}{cola($\alpha$)}{nat}{}
...
\tadAxioma{pr\'oximo(encolar($e$,$c$))}{\IF vacia?($c$) THEN $e$ ELSE pr\'oximo($c$) FI}

\tadAxioma{desencolar(encolar($e$,$c$))}{\IF vacía?($c$) THEN vacía ELSE encolar($e$, desencolar($c$)) FI}

\tadAxioma{tama\~no($c$)}{\IF vac\'ia?($c$) THEN 0 ELSE 1 + tama\~no(desencolar($c$)) FI}

\end{tad}

\section{TAD \tadNombre{\'Arbol binario($\alpha$)}}

\begin{tad}{\tadNombre{\'Arbol binario($\alpha$)}}
...
\tadGeneros{ab($\alpha$)}
\tadExporta{ab($\alpha$), generadores, observadores, tama\~no, altura, tamano, inorder, preorder, postorder}
...
\tadOperacion{tamano}{ab($\alpha$)}{nat}{}
...
\tadAxioma{tamano($a$)}{\IF nil?($a$) THEN 0 ELSE 1 + tamano(izq($a$)) + tamano(der($a$)) FI}
...
\tadIgualdadObservacional{c}{c'}{colaPrior($\alpha$)}{vacia?($c$) $\igobs$ vacia?($c'$) $\land$ \\ 
                                                 ($\neg$vacia?($c$) $\implies$ (proximo($c$) $\igobs$ proximo($c'$) $\land$ \\
                                        \phantom{($\neg$vacia?($c$) $\implies$ (}desencolar($c$) $\igobs$ desencolar($c'$))}
...
\tadAxioma{proximo(encolar($e$, $c$))}{\IF vacía?($c$) $\lor$\ proximo($c$) $< e$ THEN $e$ ELSE proximo($c$) FI}
