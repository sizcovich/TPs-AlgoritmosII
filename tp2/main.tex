...
\begin{enumerate}
  ...
  {\bf Formalmente:} $(\exists n:nat)acotado(n,e,e.raiz)$
    
\end{enumerate}

\tadOperacion{acotado}{cota X abcat X nombre}{Bool}{}
  \tadAxioma{(n, ab, cat)}{\IF $n = 0$ THEN false ELSE ($\forall$ h:string)(h $\in$ hijas(abcat, cat)) acotado(n-1, abcat, h) FI}
