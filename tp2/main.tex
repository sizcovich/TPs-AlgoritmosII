...
\Statex	En este auxiliar y en los auxiliares que llama, supongo que la clave
 existe.
...
\tadAxioma{dameSignifAux(a, n, p)}{\IF n = longitud(a) THEN hijo(p, a[n]).elem
...
\tadAxioma{hijoChar(p, c)}{\IF (*p).hijo.letra = c THEN (*p).hijo ELSE
hermanoChar((*p).hijo, c) FI}
~
\tadAlinearAxiomas{hermanoChar(p, c)}
\tadOperacion{hermanoChar}{puntero(nodo), char}{puntero(nodo)}{}
\tadAxioma{hermanoChar(p, c)}{\IF (*p).hermano.letra = c THEN (*p).hermano
