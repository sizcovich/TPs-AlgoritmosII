...
\begin{algorithmic}[1]
	...
	\State $res.linksPorCat \gets$ Vacia()\Comment{$\Theta$(1)}
	\State $res.infoLinks \gets$ Vacio()\Comment{$\Theta$(1)}
	\\
	\For {$i=0$ to $(ac.ultID-1)$} \Comment{$n * \Theta$(1) = $\Theta$(n)}
	\State AgregarAtras($res.lisnkPorCat$, Vacia()) \Comment{$\Theta$(1)}
	\EndFor
\end{algorithmic}
...
\begin{algorithmic}[1]
	\State $categoria \gets$ Obtener($s.aCategorias, c$)\Comment{O(|c|)}
	\State AgregarAtras$(s.linksPorCat[(categoria.id-1)], l)$\Comment{O(1) + O(|$l$|) = O(|$l$|)}
	\Statex
	\var{info}{infoLink}
	\State $info.categoria \gets c$\Comment{O(|c|)}
	\State $info.accesos \gets $Vacia()\Comment{O(1)}
	\State Definir$(s.infoLinks, l, info)$\Comment{O(|$l$|)}
\end{algorithmic}
...
\begin{algorithmic}[1]
	\State $link \gets$ Obtener$(s.infoLinks, l)$\Comment{O(|$l$|)}
	\State AgregarAtras$(link.accesos, f)$\Comment{O(1)}
\end{algorithmic}
...
\begin{Interfaz}
	...
	\InterfazFuncion{crearIterador}{\In{l}{lista($\alpha$)}}{iteradorUni($\alpha$)}
	  ...
	  [O(1)]
	  [Crea un iterador unidireccional a la listade forma tal que al pedir siguiente se obtenga el primer elemento de l]

	\InterfazFuncion{HayMas?}{\In{it}{iteradorUni($\alpha$)}}{Bool}
	  {$res \igobs HayMas?(it)$}
	  [O(1)]
	  [Devuelve true si y solo si en el iterador todav\'ia quedan elementos para recorrer]
	...
	\InterfazFuncion{Siguiente}{\Inout{it}{iteradorUni($\alpha$)}}{$\alpha$}
	  ...
	  [O(1)]
	  [Avanza al iterador a la posici\'on y la devuelve por referencia, pero dicho elemento no debe ser modificado ya que modificaria la lista y eso no esta permitido]
