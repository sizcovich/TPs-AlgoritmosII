...
\AbsFc[estr]{diccString($\alpha$)}{t:diccString($\alpha$) $|$ claves(t) = eClavesTrie(e.trie, "") $\land$ ($\forall$c:categoria)c $\in$ claves(t) $\impluego$ def?(c, d) $=_{obs}$ c $\in$ eClavesTrie}
