...
\begin{algorithmic}[1]
	...
	\State \Call{AgregarAtras}{$s.linksPorCat[(ID(s.aCategorias,c)-1)], l$}\Comment{O(1) + O(|$c$|) + O(|$l$|) = O(|$l$| + |$c$|)}
	\Statex
	\var{info}{infoLink}
...
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $O(1) + O(|l| + |c|) + 2O(|c|) + O(|l|) = O(|c|) + O(|l|) + O(|l| + |c|) = 2O(|c| + |l|) = O(|c| + |l|)$
