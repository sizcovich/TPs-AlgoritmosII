...
\modTitulo{Operaciones del iterador}
	...
	\InterfazFuncion{CrearItLinks}{\In{ls}{lista(infoLink)}, \In{f}{fecha}, \In{s}{LinkLinkIt}, \In{c}{categoria}}{itLinks}
	  [$(\forall\ l:infoLink)(\emph{est\'a}?(l, ls) \Rightarrow l.ultAcceso \leq f \land esSubCategoria(s.aCategorias, c, l.categoria))$]
	  {$res \igobs CrearItUni(secuInfoLinks(s, c))$}
	  [$O(1)$]
...
cjtoLinks(s.infoLinks) $\land$ ($\forall$ l:link) l $\in$ links(s) \impluego
categoriaLink(s, l) = obtener(l, e.linksPorCat).categoria $\land$ fechaActual(s)
= mayorUltimoAcceso $\land$ ($\forall$ k:link) k $\in$ links(s) $\impluego$
fechaUltimoAcceso(s, k) = obtener(e.infoLinks, k).ultAcceso $\land$  ($\forall$
l:link) l $\in$ links(s) \impluego ($\forall$ f:fecha)
accesosRecientesDia(s, l, f) $\impluego$ } 
~
\tadAlinearFunciones{cjtoLinks}{diccString(infolink)}
\tadAlinearAxiomas{cjtoLinks(a)}
\tadOperacion{cjtoLinks}{diccString(infolink)}{conj(link)}{}
\tadAxioma{cjtoLinks(a)}{}
~
\tadAlinearFunciones{mayorUltimoAcceso}{Diccstring}
\tadAlinearAxiomas{mayorUltimoAcceso}
\tadOperacion{mayorUltimoAcceso}{DiccString}{fecha}{}
\tadAxioma{mayorUltimoAcceso(d)}{}
...
\tadAlinearFunciones{Abs}{estr\ e}
\tadAlinearAxiomas{Abs(e)}
\AbsFc{itUni}{it:itUni $|$ Siguientes(it) $\igobs$ }
...
\begin{algorithm}[H]
\caption*{iCrearItLinks(\In{ls}{lista(infoLink)}, \In{fecha}{fecha}, \In{s}{LinkLinkIt}, \In{c}{categoria}) $\DRef res$ : \TipoVariable{\hyperlink{itLinks}{itLinks}}}
\begin{algorithmic}[1]
