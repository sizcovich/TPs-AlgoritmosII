...
%        (*q).letra \neq (*p).letra$
  ...
  \item No existen dos hermanos con la misma letra\\
        ...
        ePunteros(e.trie))\ sinRepetidos(letrasHermanos(p))$
...
eClavesTrie(*(e.trie), c))) $\land$ ($\forall$ d:\ vector(char), def?(d, e))
obtener(d, e) $\Leftrightarrow$ }
~
...
\item $hijo$ y $hermano$ son punteros que nos sirven para poder recorrer el trie e ir formando los strings que se van almacenando en el trie. \\ \tab $hermano$ nos dice que esos dos caracteres(tanto el que se representa con el nodo que lo contiene como el nodo al que est\'e apunta) comparten el mismo string como prefijo.\\
\tab $hijo$ nos dice que el string continua (en el caso de que $hijo$ no sea $NULL$) o que el string finaliza ah\'i (en el caso contrario).\\
