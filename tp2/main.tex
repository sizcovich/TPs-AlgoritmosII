...
\tadAlinearFunciones{Abs}{estr\ e}
\tadAlinearAxiomas{Abs(e)}
\AbsFc{lli}{l : lli $|$ categorias(l) = e.categorias $\land$ links(l) = cjtoLinks() $\land$ ($\forall$ link:link)link $\in$ links(l) \impluego categoriaLink(l,link) = obtener(link, e.linksPorCat).categoria $\land$ fechaActual(l) = mayorUltimoAcceso $\land$ ($\forall$ link:link)link $\in$ links(l) $\impluego$ fechaUltimoAcceso(l,link) = obtener(e.infoLinks, link).ultAcceso $\land$  ($\forall$ link:link)link $\in$ links(l) \impluego ($\forall$ fecha:nat) accesosRecientesDia(l, link, fecha) $\impluego$ }
~
\tadAlinearFunciones{cjtoLinks}{acat}
\tadAlinearAxiomas{cjtoLinks(a)}
\tadOperacion{cjtoLinks}{acat}{conj(link)}{}
