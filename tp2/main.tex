...
\begin{Interfaz}
	...
	\InterfazFuncion{padres}{\In{ac}{abCat}, \In{hija}{categoria}}{iteradorUni(Nat)}
	  [est\'a?(hija, ac)]
	  {$res \igobs crearItUni(padres(ac, hija))$}
	  [$\Theta(|hija|)$]
	  [Devuelve un iterador que proyecta los ids de los padres de forma tal que al pedir actual, el iterador devuelve el id de la hija. Lo bueno es que una vez creado, acceder al id del padre y subir es todo O(1)]
...
\tadAlinearAxiomas{Rep[abcat]}
\Rep[e][abcat]{1 $\yluego$ 2 $\land$ 3 $\yluego$ 4 $\yluego$ 5 $\yluego$ 6 $\yluego$ 7 $\yluego$ 8 $\yluego$ 9}\mbox{}

$\bold{Donde:}$

$\bf\underline{1 es:}$\newline
$\bold{Coloquialmente:}$ El id de la raíz del árbol es 1 (requerido por el enunciado del TP).\newline
$\bold{Formalmente:}$ id(raiz) $=$ 1
	
$\bf\underline{2 es:}$\newline
$\bold{Coloquialmente}$: No existe ninguna categoría incluida en sus categorías hijas.\newline
$\bold{Formalmente:}$ ($\forall$c:categoria)estaAbcat(c,e) $\Rightarrow$ $\neg$(estaLista(c,hijas(c)))

$\bf\underline{3 es:}$\newline
$\bold{Coloquialmente:}$ Todas las categorías tienen padre, excluyendo la raíz.\newline
$\bold{Formalmente:}$ ($\forall$c:categoria)(estaAbcat(c,e) $\land$ (c $\not$= raiz)) $\Rightarrow$ (($\exists$t:categoria)(estaAbcat(t,e) $\land$ t$\not$=c) $\yluego$ estaLista(c,hijas(t)))    

$\bf\underline{4 es:}$\newline
$\bold{Coloquialmente:}$ Existe una categoría que no tiene padre y que es igual a raíz.\newline
$\bold{Formalmente:}$ ($\exists$t:categoria)((estaAbcat(t,e)) $\land$ (t = raiz)) $\yluego$ ($\neg$($\exists$c:categoria)estaAbcat(c,e) $\yluego$ estaLista(t,hijas(c)))

$\bf\underline{5 es:}$\newline
$\bold{Coloquialmente:}$ Cada categoría tiene un único padre (este ítem (junto con el 4) garantiza que el árbol no sea cíclico).\newline
$\bold{Formalmente:}$ $\neg$($\exists$t,c:categoria)((estaAbcat(c,e)) $\yluego$ (estaAbcat(t,e)) (necesario???) $\yluego$ (t$\not$=c)) $\yluego$ (($\forall$k:categoria)estaAbcat(k,e) $\Rightarrow$ (estaLista(k,hijas(c)) $\yluego$ estaLista(k,hijas(t)))

$\bf\underline{6 es:}$\newline
$\bold{Coloquialmente:}$ La cantidad de categorías totales es igual a UltId.\newline
$\bold{Formalmente:} \#$conjuntoCats(e) = UltId  

$\bf\underline{7 es:}$\newline
$\bold{Coloquialmente:}$ Las categorías hijas de cada categoría tienen un id mayor a ésta.\newline
$\bold{Formalmente:}$ (($\forall$c:categoria)estaAbcat(c,e)) $\Rightarrow$ (($\forall$t:categoria)(estaAbcat(c,e) $\yluego$ estaLista(t,hijas(c))) $\Rightarrow$ id(c) $<$ id(t))

$\bf\underline{8 es:}$\newline
$\bold{Coloquialmente:}$ No existen dos id's iguales para categorías distintas.\newline
$\bold{Formalmente:}$ ($\forall$c,t:categoria)(estaAbcat(c,e) $\land$ estaAbcat(t,e) $\land$ t$\not$=c) $\Rightarrow$ id(c)$\not$=id(t) 

$\bf\underline{9 es:}$\newline
$\bold{Coloquialmente:}$ No existen dos categorías iguales en un mismo árbol.\newline
$\bold{Formalmente:}$ ($\forall$c:categoria)estaAbcat(c,e) $\Rightarrow$ $\neg$($\exists$t:categoria)estaAbcat(t,e) (esto lo tengo que preguntar)
...
\begin{algorithmic}[1]
	...
	\State \Call{AgregarAtras}{$res.linksPorCat, \Call{Vacia}{\null}$}\Comment{como son llamadas consecutivas, $\Theta$(1)}
	\EndFor
\end{algorithmic}
...
\begin{algorithmic}[1]
	...
	\While{\Call{HayMas?}{$it$}}\Comment{$(O(h) + O(1)) * 3O(1) = O(h)$}
		\State \Call{Subir}{$it$}\Comment{O(1)}
    	\State $categoriaID \gets$ \Call{ActualID}{$it$}\Comment{O(1)}
		\State \Call{AgregarAtras}{$s.linksPorCat[categoriaID-1], l$}\Comment{por ref. $\Rightarrow$ O(1)}
	\EndWhile
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $2O(1) + 3O(|c|) + O(|l|) + O(h) = O(|c|) + O(|l|) + O(h) = O(|c| + |l| + h)$
...
\begin{algorithmic}[1]
	\State $categoriaID \gets$ \Call{ID}{$s.aCategorias, c$}\Comment{$O(|c|)$}
	\State $res \gets \Call{Longitud}{s.linksPorCat[categoriaID-1]}$\Comment{$2\Theta(1) = \Theta(1)$}
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $O(|c|) + \Theta(1) = O(|c|)$
\end{algorithm}
