...
\begin{enumerate}
  \item Todos los nodos cuyo hijo sea NULL, tienen elem distinto de NULL.\\
        {\bf Formalmente:} $(\forall p:puntero(nodo))(p \in ePunteros(<>, e.trie) \Rightarrow p.hijo = NULL \Rightarrow p.elem \neq NULL)$

  \item No existen dos \TipoVariable{punteros(nodo)} que apunten al mismo.\\
...
\subsection{Interfaz}
    ...
    \InterfazFuncion{puntajeDelLink}{\In{il}{infoLink}, \In{f}{fecha}}{Nat}
	  [$il.ultAcceso \leq f$]
	  {$res \igobs accesosRecientes($?`$s?, infoLink.categoria, infoLink.link)$}
	  [$\Theta(1)$]
...
\begin{algorithmic}[1]
	...
	\EndWhile
	\If{$estaOrdenada$}
		\State \Return $res \gets \Call{crearItLinks}{lista, f}$\Comment{O(1)}
	\EndIf
	\Statex
	...
	\State $res \gets \Call{crearItLinks}{listaOrdenada, f}$\Comment{O(1)}
\end{algorithmic}
...
\begin{algorithm}[H]
\caption*{iCrearItLinks(\In{ls}{lista(link)}, \In{fecha}{fecha}) $\DRef res$ : \TipoVariable{\hyperlink{itLinks}{itLinks}}}
\begin{algorithmic}[1]
...
\begin{algorithmic}[1]
	\State $res \gets \Call{HaySiguiente?}{itl.it} \yluego \Call{puntajeDelLink}{\Call{Siguiente}{itl.it}, itl.utlAcceso} > 0$\Comment{3O(1) = O(1)}
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $O(1)$
\end{algorithm}
...
\tadAlinearFunciones{Abs}{estr\ e}
\tadAlinearAxiomas{Abs(e)}
\AbsFc{iteradorUni($\alpha$)}{it:iteradorUni($\alpha$) $|$ Siguientes(it) $\igobs$ Siguientes(e.itList)}
