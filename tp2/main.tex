...
C\'alculo de complejidad:
\\
\\3O(1) + O(|$hija$|) + O(|$padre$|) + O(|$hija$|) =
\\2O(|$hija$|) + O(|$padre$|) =
\\O(|$hija$|) + O(|$padre$|) =
\\O(|$hija$| + |$padre$|)
\\
\begin{algorithmic}[1]
\Procedure{iHijas}{\In{ac}{abCat}, \In{padre}{categoria}}{ $\DRef res$ : \TipoVariable{iteradorUni(categoria)}}
...
\begin{algorithmic}[1]
\Procedure{iAgregarLink}{\Inout{s}{LinkLinkIt}, \In{l}{link}, \In{c}{categoria}}{LinkLinkIt}
%	\State $res \gets ac.raiz$
\EndProcedure
\end{algorithmic}

\begin{algorithmic}[1]
\Procedure{iAccederLink}{\Inout{s}{LinkLinkIt}, \In{l}{link}, \In{f}{fecha}}{LinkLinkIt}
%	\\
%	\var{infoHija}{catInfo}
...
\begin{algorithmic}[1]
\Procedure{crearIterador}{\In{l}{lista($\alpha$)}}{ $\DRef res$ : \TipoVariable{iteradorUni($\alpha$)}}
	\State $res.lista \gets l$\Comment{O(1)}
\EndProcedure
\end{algorithmic}

\begin{algorithmic}[1]
\Procedure{HayMas?}{\In{it}{iteradorUni($\alpha$)}}{ $\DRef res$ : \TipoVariable{Bool}}
	\State $res \gets \neg $EsVac\'ia?($it.lista$)\Comment{$\Theta(1)$}
\EndProcedure
\end{algorithmic}

\begin{algorithmic}[1]
\Procedure{Siguiente}{\Inout{it}{iteradorUni($\alpha$)}}{ $\DRef res$ : \TipoVariable{$\alpha$}}
