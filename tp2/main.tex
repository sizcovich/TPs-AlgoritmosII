...
categoriaLink(s, l) = obtener(l, e.linksPorCat).categoria $\land$ fechaActual(s)
= mayorUltimoAcceso(indice(s.linksPorCat, 1)) $\land$ ($\forall$ k:link) k $\in$
links(s) $\impluego$ fechaUltimoAcceso(s, k) = obtener(e.infoLinks, k).ultAcceso $\land$  ($\forall$
l:link) l $\in$ links(s) \impluego ($\forall$ f:fecha)
accesosRecientesDia(s, l, f) $\impluego$}
~
\tadAlinearFunciones{mayorUltimoAcceso}{lista(puntero(infoLinik))}{fecha}
\tadAlinearAxiomas{mayorUltimoAcceso(list)}
\tadOperacion{mayorUltimoAcceso}{lista(puntero(infoLinik))}{fecha}{}
\tadAxioma{mayorUltimoAcceso(list)}{\IF list $\neq$ <>
