...
\begin{Interfaz}
	...
	\InterfazFuncion{padreID}{\In{ac}{abCat}}{Nat}
	  {$res \in \TipoVariable{Nat}$}
	  [$\Theta(1)$]
	  [Devuelve el id del padre de la \'ultima categoria a la cual se le pregunt\'o el id.]
...
\begin{Representacion}
	...
	\begin{Estructura}{abCat}
	    \begin{Tupla}
	      ...
	      \tupItem{ultID}{Nat}
	      \tupItem{\\ ultCatAccedida}{puntero(catInfo)}
	    \end{Tupla}
...
\begin{algorithmic}[1]
	...
	\State $infoPadre.padre \gets \TipoVariable{NULL}$\Comment{O(1)}
...
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $8O(1) + 2O(|raiz|) = O(|raiz|)$
\end{algorithm}
...
\begin{algorithm}[H]
\caption*{iPadreID(\Inout{ac}{abCat}, \In{hija}{categoria}) $\DRef res$ : \TipoVariable{Nat}}
\begin{algorithmic}[1]
	\State $res \gets (*ac.ultCatAccedida.padre).id$\Comment{O(1)}
	\State $ac.ultCatAccedida \gets \&ac.ultCatAccedida.padre$\Comment{O(1)}
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $2O(1) = O(1)$
\end{algorithm}
...
\begin{algorithm}[H]
\caption*{iID(\Inout{ac}{abCat}, \In{c}{categoria}) $\DRef res$ : \TipoVariable{Nat}}
\begin{algorithmic}[1]
	\State $ac.ultCatAccedida \gets$ \Call{Obtener}{$ac.categorias, c$}\Comment{O(|$c$|)}
	\State $res \gets ac.ultCatAccedida.id$\Comment{O(1)}
\end{algorithmic}
...
\begin{algorithmic}[1]
	...
	\Statex
	\While{$categoriaID > 0$}\Comment{$O(h) * 2O(1)$}
		\State \Call{AgregarAtras}{$s.linksPorCat[categoriaID-1], l$}\Comment{O(1) ya que se est\'a pasando por referencia}
    	\State $categoriaID \gets$ \Call{padreID}{s}\Comment{O(1)}
	\EndWhile
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $O(1) + 2O(|c|) + 2O(|l|) + O(h) = O(|c|) + O(|l|) + O(h) = O(|c| + |l| + h))$
\end{algorithm}
