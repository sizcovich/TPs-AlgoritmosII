...
\begin{enumerate}
  \item Todos los nodos cuyo hijo sea NULL, tienen elem distinto de NULL.\\
        {\bf Formalmente:} $(\forall p:puntero(nodo), p \in ePunteros(e.trie))
        \ ((*p).hijo = NULL) \Rightarrow (*p).elem \neq NULL$
...
%        (*q).letra \neq (*p).letra$
  ...
  \item No existen dos hermanos con la misma letra\\
        ...
        ePunteros(e.trie))\ esta?((*p).letra,\ letrasHermanos(p))$
...
~
\tadAlinearAxiomas{lertasHermanos(p)}
\tadOperacion{lertasHermanos}{puntero(nodo)}{secu(puntero(nodo))}{}
\tadAxioma{lertasHermanos(p)}{\IF p = NULL THEN <> ELSE
lertasHermanosAux((*p).hermano)) FI} 
~
\tadAlinearAxiomas{lertasHermanosAux(p)}
\tadOperacion{lertasHermanosAux}{puntero(nodo)}{secu(puntero(nodo))}{}
\tadAxioma{lertasHermanosAux(p)}{\IF p = NULL THEN <> ELSE p.letra $\bullet$
lertasHermanosAux((*p).hermano)) FI} 
~
\tadAlinearAxiomas{ePunteros(p)}
\tadOperacion{ePunteros}{puntero(nodo)}{secu(puntero(nodo))}{}
...
\AbsFc{diccString($\alpha$)}{($\forall$ c:string)def?(c, t) $\Leftrightarrow$ ((e.trie $\not=$ NULL) $\impluego$ (c $\in$ eClavesTrie(*(e.trie), c)))) $\land$ ($\forall$ )}
~
...
$\bullet$ $trie$ es un puntero al primer char del primer string añadido.\\
$\bullet$ $hijo$ es un puntero al hijo del char.\\
$\bullet$ $hermano$ es un puntero al hermano del char.\\
$\bullet$ $elem$ es un puntero al significado de la palabra, por lo que el $elem$ de un char puede o no ser NULL (dependiendo de si la palabra lleg\'o o no a su fin).\\
$\bullet$ $letra$ es el char al que apunta el puntero $trie$.\\
...
\begin{itemize}
	...
	\item $raiz$ es la categor\'ia raiz del Trie.
	\item $ultId$ es el $id$ de la \'ultima categor\'ia agregada al Trie.
	...
	\item $hijas$ es la lista de hijas directas de la categor\'ia.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Todo infoLink en linksPorCat debe estar definido en infoLinks con la clave igual a su link.\\
		{\bf Formalmente:} $(\forall pil:puntero(infoLink))(pil \in ePunterosInfoLink(e.linksPorCat) \impluego (pil \neq NULL \yluego \&obtener(e.infoLinks, (*pil).link) \igobs pil))$
