...
\begin{Representacion}
	...
	\begin{Estructura}{diccTrie($\alpha$)}
	    ...
	    \begin{Tupla}[nodo]
	      \tupItem{hijo}{vector(puntero(nodo))}
...
\begin{algorithmic}[1]
	\var{nodoVacio}{nodo}	
	\State $nodoVacio.elem \gets NULL$ \Comment{O(1)}
	\State $nodoVacio.hijo \gets$ \Call{vacia}{} \Comment{O(1)}
	\State $i \gets 0$\Comment{O(1)}
	\While{$i < 256$}\Comment{$O(256)* O(1) = O(256) = O(1)$}
    	\State \Call{AgregarAtras}{$nodoVacio.hijo, NULL$}\Comment{O(1)}
	\EndWhile
	\State $res.trie \gets \&nodoVacio$ \Comment{O(1)}
\end{algorithmic}
...
\begin{algorithmic}[1]
	...
	\For {$i = 0 \to (\Call{longitud}{k} - 1)$}\Comment{$O(256)* O(1) = O(256) = O(1)$}
    	\If{$nodo.hijo[\Call{ord}{k[i]}] \neq NULL$}
		...
		\Else
			...
			\While{$j < 256$}\Comment{$O(256)* O(1) = O(256) = O(1)$}
    			\State \Call{AgregarAtras}{$nodoVacio.hijo, NULL$}\Comment{O(1)}
			\EndWhile
			\State $nodo.hijo[\Call{ord}{k[i]}] \gets \&nodoVacio$\Comment{O(1)}
...
\begin{algorithmic}[1]
	...
	\State $nodo \gets *d.trie$\Comment{O(1)}
	\For {$i = 0 \to (\Call{longitud}{k} - 1)$}\Comment{$O(256)* O(1) = O(256) = O(1)$}
    	\If{$nodo.hijo[\Call{ord}{k[i]}] = NULL$}
			\State $res \gets False$
		\Else
			\State $nodo \gets *nodo.hijo[\Call{ord}{k[i]}]$\Comment{O(1)}
		\EndIf
	\EndFor
...
\tadAlinearAxiomas{Rep(e)}
\Rep[abcat][e]{$1 \land 2 \yluego 3 \yluego 4 \yluego 5 \yluego 6 \land 7 \land 8 \land 9 \land 10$}\mbox{}
...
\item No existen dos categor\'ias iguales en un mismo \'arbol.\\
        {\bf Formalmente:} $(\forall c:categoria)esta?(c,e) \Rightarrow \neg(\exists t:categoria)esta?(t,e)$ (esto lo tengo que preguntar)
  ...
  %\item Ningún padre tiene más de dos hijos diferentes. NO, NO ES BINARIO
...
\begin{Interfaz}
    ...
    \InterfazFuncion{puntajeDelLink}{\In{il}{infoLink}, \In{f}{fecha}}{Nat}
	  ...
	  {$res \igobs ???$}
	  [$\Theta(1)$]be
	  [Devuelve el puntaje del link. Esta funci\'on es privada, ya que reci] una estructura interna.
...
\begin{algorithmic}[1]
    \State $categoriaID \gets \Call{ID}{s.aCategorias, c}$\Comment{$O(1)$}
    \State $it \gets \Call{crearIt}{s.linksPorCat[categoriaID - 1]}$\Comment{$O(1)$}
    \Statex
    ...
    \EndWhile
    \Statex
    \Statex // Filtro los que tienen que quedar (aquellos que tengan alg\'un acceso)
    \State $itLinksEnCat \gets \Call{crearIt}{s.linksPorCat[categoriaID - 1]}$
	\State $listaRes \gets \Call{Vacia}{\null}$
	\State $itRes \gets \Call{crearIt}{listaRes}$
	\While{$\Call{HaySiguiente}{itLinksEnCat}$}
		\If{$\Call{puntajeDelLink}{\Call{Siguiente}{itLinksEnCat}, f} > 0$}
			\State $\Call{AgregarComoAnterior}{itRes, \Call{Siguiente}{itLinksEnCat}}$
		\EndIf
	\EndWhile
	\Statex
	\Statex // Voy agregando el de m\'as accesos que encuentre
	\State $listaResOrdenada \gets \Call{Vacia}{\null}$
	\State $itResOrd \gets \Call{crearIt}{listaResOrdenada}$
	\While{$\neg\Call{Vacia}{listaRes}$}
		\State $itRes \gets \Call{crearIt}{listaRes}$
		\State $itMaxAccesos \gets \Call{crearIt}{listaRes}$
		\While{$\Call{HaySiguiente}{itRes}$}
		...
		\EndWhile
		\Statex
		\State $\Call{AgregarComoAnterior}{itResOrd, \Call{Siguiente}{itMaxAccesos}}$
		\State $\Call{EliminarSiguiente}{itMaxAccesos}$
	\EndWhile
	\Statex
	\State $res \gets \Call{crearItLinks}{listaResOrdenada}$
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $O(|c| + n^2)$
\end{algorithm}
