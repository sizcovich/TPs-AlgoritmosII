...
\item No existen dos categor\'ias iguales en un mismo \'arbol.\\
  ...
  \item Cada categor\'ia tiene un \'unico padre (este \'item (junto con el 4) garantiza que el \'arbol no sea c\'iclico). (esto supuestamente queda explicito pues el padre es un puntero)\\
  ...
  \item Ningún padre tiene más de dos hijos diferentes.
...
\begin{algorithmic}[1]
    ...
    \EndWhile
    \Statex
    \Statex // Ni idea, hay que ordenar, hoy es el taller de sorting, ya veremos\dots
    \If{$\Call{HaySiguiente}{it}$}
        \State $\Call{Avanzar}{it}$
        \While{$\Call{HaySiguiente}{it}$}
            \If{$\Call{puntajeDelLink}{\Call{Siguiente}{it}, f} > \Call{puntajeDelLink}{\Call{Anterior}{it}, f}$}
            \Else
            \EndIf
           \Statex  //Aca hay q decir que si tiene puntaje 0 no aparece en el iterador.
           \Statex // O de otra forma, guardar la ultima posicion dnd el puntaje es diferente a 0 y pasarle esa sub-lista al it
        \EndWhile
    \EndIf
\end{algorithmic}
