...
\AbsFc{diccString($\alpha$)}{d: DiccString $|$ ($\forall$ c:\ string)(def?(c, d)
$\Leftrightarrow$ ((e.trie $\not=$ NULL)) $\impluego$ (c $\in$
eClavesTrie(*(e.trie), c))) $\land$ ($\forall$ v:\ vector(char))def?(v, d)) \impluego
(obtener(v, d) $\Leftrightarrow$ dameSignif(v,e.trie))}
~
...
\begin{itemize}
	\item $trie$ es un puntero al primer char del primer string a\~{n}adido.
	\item $hijo$ y $hermano$ son punteros que nos sirven para poder recorrer el trie e ir formando los strings que se van almacenando en el trie.
	\begin{itemize}
	  \item[hermano] nos dice que esos dos caracteres (tanto el que se representa con el nodo que lo contiene como el nodo al que \'este apunta) comparten el mismo string como prefijo.
	  \item[hijo] nos dice que el string contin\'ua (en el caso en el que $hijo$ no es $NULL$) o que finaliza ah\'i (en el caso contrario).
	\end{itemize}
...
~
\tadOperacion{infoLinks}{lli\ s, categoria\ c, secu(link)\ ls}{secu(tupla(link, categoria, Nat))}{est\'a?(c, categorias(s)) $\land$ ($\forall$ l:link)(est\'a?(l, ls) $\Rightarrow$ l $\in$ links(s) $\land$ esSubCategoria(s.aCategorias, c, categoriaLink(s.aCategorias, l)))}
...
\modTitulo{Operaciones del iterador}
	...
	\InterfazFuncion{CrearItLinks}{\In{ls}{lista(puntero(infoLink))}, \In{f}{fecha}}{itLinks}
	  [$(\forall\ l:infoLink)(\emph{est\'a}?(l, ls) \Rightarrow l.ultAcceso \leq f)$]
	  {$res \igobs CrearItUni(eInfoLinksAUniTupla(ls, f))$}
	  [$O(1)$]
	  [Genera un iterador de links que permite verse a si mismo, observar su categor\'ia y la cantidad de accesos recientes. eInfoLinksAUniTupla est\'a axiomatizado en la p\'agina~\pageref{eInfoLinksAUniTupla}.]
...
\tadAlinearAxiomas{Rep(e)}
\Rep{$1 \land 2 \land (\forall pil:puntero(infoLink))(pil \in aplanar(e.linksPorCat) \impluego 3 \yluego 4)$}\mbox{}
...
\begin{enumerate}
  ...
  \item Todo infoLink en linksPorCat debe obtenerse y ser el mismo en infoLinks con la clave igual a su link.\\
...
\tadAlinearFunciones{eCategoriasPadres}{abCat\ ac, infoLink\ il}
\tadAlinearAxiomas{eCategoriasPadres(ac, il)}
\tadOperacion{eCategoriasPadres}{abCat\ ac, infoLink\ il}{conj(Nat)}{il.categoria $\in$ categorias(ac)}
\tadAxioma{eCategoriasPadres(ac, il)}{Ag(id(ac, il.categoria), \IF il.padre $\neq$ NULL THEN eCategoriasPadres(*il.padre) ELSE $\emptyset$ FI)}
~
\tadAlinearFunciones{aplanar}{secu(secu($\alpha$))}
\tadAlinearAxiomas{aplanar(ss)}
\tadOperacion{aplanar}{secu(secu($\alpha$))}{secu($\alpha$)}{}
\tadAxioma{aplanar(ss)}{\IF vacia?(ss) THEN <> ELSE prim(ss) \& aplanar(fin(ss)) FI}
~
\tadAlinearFunciones{indice}{secu($\alpha$)\ s, Nat\ i}
\tadAlinearAxiomas{indice(s, i)}
\tadOperacion{indice}{secu($\alpha$)\ s, Nat\ i}{$\alpha$}{i < \#s}
\tadAxioma{indice(s, i)}{\IF i = 0 THEN prim(s) ELSE indice(fin(s), i - 1) FI}
~
\tadAlinearFunciones{Abs}{estr\ e}
\tadAlinearAxiomas{Abs(e)}
\AbsFc{lli}{s : lli $|$ categorias(s) = e.categorias $\land$\\ links(s) =
claves(s.infoLinks) $\land$\\ ($\forall$ l:link) l $\in$ links(s) \impluego
categoriaLink(s, l) = obtener(l, e.linksPorCat).categoria $\land$\\
fechaActual(s) = mayorUltimoAcceso(indice(s.linksPorCat, 1)) $\land$\\ ($\forall$ k:link) k
$\in$ links(s) $\impluego$ fechaUltimoAcceso(s, k) = obtener(e.infoLinks, k).ultAcceso
$\land$\\  ($\forall$ l:link) l $\in$ links(s) \impluego (($\forall$ f:fecha)
accesosRecientesDia(s, l, f) = cantAccesosEnFecha(obtener(e.infoLinks,
l), f)) $\land$\\
($\forall$ k: link) k $\in$ links(s) \impluego categoriaLink(s, k) =
obtener(e.infoLinks, k).categoria}
~
\tadAlinearFunciones{mayorUltimoAcceso}{lista(puntero(infoLinik))}{}
\tadAlinearAxiomas{mayorUltimoAcceso(list)}
\tadOperacion{mayorUltimoAcceso}{lista(puntero(infoLinik))}{fecha}{}
\tadAxioma{mayorUltimoAcceso(list)}{\IF vacia?(list)
THEN max(prim(list).ultAcceso, mayorUltimoAcceso(fin(list))) ELSE 0 FI}
~
\tadAlinearFunciones{cantAccesosEnFecha}{infoLink, fecha}{}
\tadAlinearAxiomas{cantAccesosEnFecha(info, f)}
\tadOperacion{cantAccesosEnFecha}{infoLink, fecha}{nat}{}
\tadAxioma{cantAccesosEnFecha(info, f)}{\IF (f < info.ultAcceso -2) $\lor$ (f >
info.ultAcceso) THEN 0 ELSE indice(info.accesos,f-info.ultAcceso) FI}
~
\modTitulo{Justificaci\'on de la elecci\'on de la estructura}
...
\tadAlinearFunciones{Rep}{estr}
\tadAlinearAxiomas{Rep(e)}
\Rep{$(\forall p:puntero(infoLink)) (p \in Siguientes(e.it) \impluego (1 \yluego 2))$}\mbox{}
...
\tadAlinearFunciones{Abs}{estr\ e}
\tadAlinearAxiomas{Abs(e)}
\AbsFc{itUni}{it:itUni $|$ Siguientes(it) $\igobs$ eInfoLinksAUniTupla(Siguientes(e.it), e.ultAcceso)}
~
\tadAlinearFunciones{eInfoLinksAUniTupla}{secu(puntero(infoLink))\ spil, fecha\ f}
\tadAlinearAxiomas{eInfoLinksAUniTupla(spil, f)}
\label{eInfoLinksAUniTupla}
\tadOperacion{eInfoLinksAUniTupla}{secu(puntero(infoLink))\ spil, fecha\ f}{secu(tupla(link, categoria, Nat))}{$(\forall pil:puntero(infoLink))(esta?(pil, spil) \Rightarrow (pil \neq NULL \yluego (*pil).ultAcceso \leq f))$}
\tadAxioma{eInfoLinksAUniTupla(spil, f)}{\IF vacio?(spil) THEN <> ELSE tupla((*prim(spil)).link, (*prim(spil)).categoria, puntajeDelLink(*prim(spil), f)) $\bullet$ eInfoLinksAUniTupla(fin(spil), f) FI}

\modTitulo{Justificaci\'on de la elecci\'on de la estructura}

Esta estructura contiene una lista de links para los cuales se puede saber su categor\'ia y su ``accesosRecientes'', uno por uno. Para este \'ultimo dato, necesitamos tener una copia del sistema y saber la \'ultima fecha en la cual se accedi\'o alguno de estos links, \'esta la guardamos en $utlAcceso$ para poder tenerla r\'apidamente.

\modTitulo{Operacion Auxiliar}


\InterfazFuncion{puntajeDelLink}{\In{il}{infoLink}, \In{ultAccesoCat}{fecha}}{Nat}
  [$il.ultAcceso \leq f$]
  {$res \igobs puntajeDelLink(il, ultAccesoCat)$}
  [$\Theta(1)$]
  [Devuelve el puntaje del link. Esta funci\'on es privada ya que recibe una estructura interna.]
\\
\tadAlinearFunciones{puntajeDelLink}{infoLink\ il, fecha\ f}
\tadAlinearAxiomas{puntajeDelLink(il, f)}
\tadOperacion{puntajeDelLink}{infoLink\ il, fecha\ f}{Nat}{il.ultAcceso $\leq$ f}
\tadAxioma{puntajeDelLink(il, f)}{\IF $il.ultAcceso + 3 > f$ THEN 0 ELSE indice(il.accesos, f - il.ultAcceso) + puntajeDelLink(il.ultAcceso, f + 1) FI}
...
\begin{algorithmic}[1]
	...
	\While{$\Call{HayMasPadres?}{it}$}\Comment{$(O(h) + O(1)) * 3O(1) = O(h)$}
    	...
    	\State $categoriaID \gets \Call{ActualID}{it}$\Comment{$O(1)$}
		\State $\Call{AgregarAtras}{s.linksPorCat[categoriaID-1], \&info}$\Comment{$O(1)$}
	\EndWhile
\end{algorithmic}
...
\begin{algorithmic}[1]
    ...
    \While{$ultAccesoCat - il.ultAcceso < i$}\Comment{gracias a la Pre $(ultAccesoCat - il.ultAcceso) > 0 \Rightarrow 3O(1) = O(1)$}
        \State $i \gets i - 1$\Comment{$O(1)$}
...
\begin{algorithmic}[1]
	...
	\State $it \gets \Call{crearIt}{lista}$\Comment{$O(1)$}
	\State $estaOrdenada \gets \TipoVariable{True}$\Comment{$O(1)$}
	\State $ultPuntaje \gets -1$\Comment{$O(1)$}
	...
	\State $itOrd \gets \Call{crearIt}{listaOrdenada}$\Comment{$O(1)$}
