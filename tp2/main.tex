...
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $3O(1) + 2O(|hija|) + O(|padre|) = O(|hija|) + O(|padre|) = O(|hija| + |padre|)$
\end{algorithm}
...
\begin{algorithmic}[1]
	...
	\Statex
	\For {$i = 1 \to ac.ultID$} \Comment{$\#categorias(ac) * \Theta(1) = \Theta(\#categorias(ac))$}
...
\begin{algorithmic}[1]
	\State $categoria \gets$ \Call{Obtener}{$s.aCategorias, c$}\Comment{O(|c|)}
	\State \Call{AgregarAtras}{$s.linksPorCat[(categoria.id-1)], l$}\Comment{O(1) + O(|$l$|) = O(|$l$|)}
	\Statex
	\var{info}{infoLink}
