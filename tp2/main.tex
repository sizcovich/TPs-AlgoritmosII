...
\end{enumerate}
  ...
  \AbsFc[estr]{abcat}{e.raiz = raiz(a) $\land$ ($\forall$ c:categoria)esta?(c,claves(e.categorias)) $\impluego$ id(e,c) = id(a,c) $\land$ ($\forall$ c:categoria)esta?(c,claves(e.categorias)) $\impluego$ *padre(e,c) = *padre(a,c) $\yluego$ ($\forall$ c:categoria)esta?(c,claves(e.categorias))  }
