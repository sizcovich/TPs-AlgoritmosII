...
\subsection{Interfaz}
	...
	\InterfazFuncion{Definido?}{\In{d}{diccTrie($\alpha$)}, \In{k}{string}}{Bool}
	  {$res \igobs def?(k, d)$}
	  [O(|$k$|)]
...
\Rep[diccTrie($\alpha$)][d]{d.letra$=_{obs}$*(d.trie) $\land$ (obtener(*(d.trie),trie)$=_{obs}$*(d.elem) \oluego obtener(*(d.trie),trie)$=_{obs}$Null) \yluego *(d.elem)$=_{obs}$obtener(d.letra)}\mbox{}

\AbsFc[estr]{diccTrie($\alpha$)}{obtener(d.letra, trie) = obtener(letra, )}

\modTitulo{Justificaci\'on de la elecci\'on de la estructura}
Decidimos realizar este m\'odulo como un trie ya que nos permite acceder a su contenido con la complejidad requerida para otros algoritmos en otros m\'odulos.
...
\par \tab El bucle, en el peor caso (donde $\neg$cargoTodo y la letra a buscar es la \'unica que me falta agregar en ese nivel), tiene 256 iteraciones y \'este, al ser un valor constante, no altera la complejidad.
\end{algorithm}

\begin{algorithm}[H]
\caption*{iDefinido?(\In{d}{diccTrie($\alpha$)}, \In{k}{string} $\DRef res$ : \TipoVariable{Bool}}
\begin{algorithmic}[1]
	\var{nodo}{nodo}
	\State $i \gets 0$\Comment{O(1)}
	\Statex
...
\par \tab La complejidad del bucle se explica en el algoritmo anterior.
\end{algorithm}
...
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $2O(1) + O(|k|) = O(|k|)$   
\par \tab La complejidad del bucle se explica en el algoritmo iDefinir.
\end{algorithm}
...
\modTitulo{Puntero ($\alpha$)}

La función * debe tener complejidad O(1).\\
La función \& debe tener complejidad O(1).

\modTitulo{Vector ($\alpha$)}

La función Longitud debe tener complejidad O(1).\\
La función $\bullet$[$\bullet$] debe tener complejidad O(1).
...
\begin{enumerate}
  ...
  {\bf Formalmente:} $*padre(e.raiz) =_{obs} Null$
  
  %\item Dada una categoría que pertenece a una lista de hijas, su categoría padre apunta al padre (necesario?)
    
\end{enumerate}

  \AbsFc[estr]{abcat}{e.raiz = raiz(a) $\land$ ($\forall$ c:categoria)esta?(c,claves(e.categorias)) $\impluego$ id(e,c) = id(a,c) $\land$ ($\forall$ c:categoria)esta?(c,claves(e.categorias)) $\impluego$ *padre(e,c) = padre(a,c) $\yluego$ categorias(a) = claves(e.categorias)}

\modTitulo{Justificaci\'on}
Para representar el \'arbol de categor\'ias elegimos esta estructura por los siguientes motivos:
\begin{itemize}
	\item En primer lugar, el Diccionario Trie modulariza perfectamente un \'arbol de categor\'ias dado que es posible conocer los hijos de cada una de las categor\'ias con la complejidad requerida, para esto, se coloco en cada nodo una lista con los nombres de cada hijo. 
	\item En segundo lugar, esta estructura nos permite acceder a la ra\'iz del arbol en O(1) y al ID de cada categor\'ia en O(|$c$|) donde $c$ es el nombre de la categor\'ia.
\end{itemize}
\par El iterador que recorre el \'arbol hacia arriba ser\'a necesario en el LinkLinkIt.
...
La función Definir debe tener complejidad O(|$k$|) donde $k$ es el string que se va a colocar por definici\'on. \\
...
La función cantCategorias debe tener complejidad O(1).\\
La función ID debe tener complejidad O(|c|) donde c es la categor\'ia de la que se desea saber el id.\\
La función padres debe tener complejidad O(|c|) donde c es la categor\'ia de la que se desea conocer sus padres.

\modTitulo{itabCat}

La función hayMasPadres? debe tener complejidad O(1).\\
La función subir debe tener complejidad O(1).\\
La función actualID debe tener complejidad O(1).

\modTitulo{diccTrie($\alpha$)}

La función definir debe tener complejidad O(|k|) donde k es la clave.\\

\modTitulo{Vector ($\alpha$)}

La función Vacia debe tener complejidad O(1).\\
La función agregarAtras debe tener complejidad O(n) s\'i y s\'olo s\'i se agregan n elementos de forma consecutiva.\\
La función Longitud debe tener complejidad O(1).\\
La función $\bullet$[$\bullet$] debe tener complejidad O(1).
...
La función Vacia debe tener complejidad O(1).\\
La función agregarAtras debe tener complejidad O($copy$($a$)), donde $a$ es el tipo del elemento que va a ser colocado en la lista.
