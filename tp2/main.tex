...
\begin{Interfaz}
	...
	\InterfazFuncion{raiz}{\In{ac}{abCat}}{categoria}
	  {$res \igobs raiz(ac)$}
	  [$\Theta(1)$]
	  [Devuelve el nombre de la categor\'ia raiz por referencia, con lo cual, si esta es modificada posteriormente desde afuera, se modificara tambien adentro del \'arbol de categor\'ias.]
...
\begin{Representacion}
	...
	\begin{Estructura}{abCat}
	    \begin{Tupla}
	      \tupItem{categorias}{DiccTrie(categoria, Nodo)}
...
Todos los pasos que no tengan especificados sus complejidades se suponen realizables en tiempos acotables por una constante.\\ 
...
\Procedure{iNuevoAC}{\In{raiz}{categoria}}{ $\DRef res$ : \TipoVariable{abCat}}
	\var{nodoRaiz}{Nodo}
	\State $nodoRaiz.id \gets 1$
	\State $nodoRaiz.hijas \gets$ Vacia()
	\State $res.categorias \gets $Vacio()
	\State Definir($res.categorias$, $raiz$, $nodoRaiz$)\Comment{O(|$raiz$|)}
...
\Procedure{iAgregar}{\Inout{ac}{abCat}, \In{padre}{categoria}, \In{hija}{categoria}}
	...
	\\
	\var{nodoHija}{Nodo}
	\State $nodoHija.id \gets ac.ultID$
	\State $nodoHija.hijas \gets$ Vacia()
	\State Definir($ac.categorias$, $hija$, $nodoHija$)\Comment{O(|$hija$|)}
	\\
	\State $nodoPadre \gets$ significado($ac.categorias$, $padre$)\Comment{O(|$padre$|)}
	\State agregarAtras($nodoPadre.hijas$, $hija$)\Comment{O(|$hija$|)}
\EndProcedure
\end{algorithmic}

\begin{algorithmic}[1]
\Procedure{iHijas}{\In{ac}{abCat}, \In{padre}{categoria}}{ $\DRef res$ : \TipoVariable{itCategorias}}
	\State $nodoHijo \gets$ significado($ac.categorias$, $padre$)\Comment{O(|$padre$|)}
...
\Procedure{iID}{\In{ac}{abCat}, \In{c}{categoria}}{ $\DRef res$ : \TipoVariable{Nat}}
	\State $nodoHijo \gets$ significado($ac.categorias$, $c$)\Comment{O(|$c$|)}
