...
\newenvironment{Operacion Auxiliar}{%
  \parskip=2ex%
  \noindent\textbf{\Large Operaci\'on auxiliar}%
  \par%
}{}
...
\begin{Operacion Auxiliar}

	\InterfazFuncion{crearNodo}{\In {c}{char}}{nodo}
	  {$res.hijo \igobs NULL \wedge res.hermano \igobs NULL \wedge res.elem \igobs NULL \wedge res.letra \igobs c$}
	  [O(1)]
	  [Genera un nodo con todos los punteros a NULL y con el char que se pasa como parametro almacenado en el campo letra]

\end{Operacion Auxiliar}
...
\begin{algorithmic}[1]
	\State $res.elem \gets NULL$ \Comment{O(1)}
	\State $res.hijo \gets NULL$ \Comment{O(1)}
	\State $res.hermano \gets NULL$ \Comment{O(1)}
...
\begin{algorithmic}[1]
	\State $res.trie \gets NULL$ \Comment{O(1)}
\end{algorithmic}
...
\begin{algorithmic}[1]
	...
	\Statex
	\If {d.trie = NULL}
		\State $cargoTodo? \gets True$\Comment{O(1)}
		\var{nodoVacio}{nodo}				
		\State $nodoVacio \gets \Call{crearNodo}{k[i]}$ \Comment{O(1)}
		\State $nodo \gets nodoVacio$ \Comment{O(1)}
		\State $i++$\Comment{O(1)}	
	\Else
		\State $cargoTodo? \gets False$\Comment{O(1)}
		\State $nodo \gets *d.trie$\Comment{O(1)}
	\EndIf	\Comment{O(1)}
	\Statex	
	\Statex // Agarre el primer nodo. si cargoTodo? es true, entonces en el bucle voy a ir creando todos los nodos hasta formar // k. Si cargoTodo? es false entonces voy a ir recorriendo todo el arbol hasta encontrar el lugar donde voy a tener // que colocar el resto de los caracteres de k que no esten en el arbol	
	\Statex
...
\\ \tab El bucle en el peor caso (cargoTodo es False y el caracter a buscar es el que tiene ord = 255 y en ese nivel estan todos los caracteres del 0 al 254) tiene que hacer 256 ciclos pero como esto es un valor finito no arruina nuestra complejidad.
\end{algorithm}
...
La función Lonitud debe tener complejidad O(1)\\
...
\begin{Interfaz}
	...
	\InterfazFuncion{hijas}{\In{ac}{abCat}, \In{padre}{categoria}}{iteradorUni(categoria)}
	  ...
	  [El iterador se invalida si y s\'olo si se elimina el elemento siguiente del iterador.]
...
\modTitulo{Operaciones del iterador}
	...
	\InterfazFuncion{Subir}{\Inout{it}{itabCat}}{}
	 ...
	 \InterfazFuncion{ActualID}{\In{it}{itabCat}}{$Nat$}
	  {$res \igobs Actual(it)$}
	  [$\Theta(1)$]
	  [Devuelve el ID del elemento al cual esta apuntando.]
...
\begin{enumerate}
  ...
  \item No existe ninguna categor\'ia incluida en sus categor\'ias hijas.\\
        {\bf Formalmente:} $(\forall c:categoria)esta?(c,e) \impluego \neg(estaLista(c,hijas(c)))$

  \item Todas las categor\'ias tienen padre, excluyendo la ra\'iz.\\
        {\bf Formalmente:} $(\forall c:categoria)(esta?(c,e) \land (c \not= e.raiz)) \impluego ((\exists t:categoria)(esta?(t,e) \land t\not=c) \yluego estaLista(c,hijas(t)))$
  ...
  4) garantiza que el \'arbol no sea c\'iclico). (esto supuestamente queda explicito pues el padre es un puntero)\\
        {\bf Formalmente:} $\neg(\exists t,c:categoria)((esta?(c,e)) \yluego (esta?(t,e))$ $\yluego (t \not= c)) \yluego ((\forall k:categoria)esta?(k,e) \impluego (estaLista(k,hijas(c)) \yluego estaLista(k,hijas(t)))$
...
\end{enumerate}
  ...
  \tadOperacion{estaLista}{lst}{bool}{} (no se como poner que me agregue un parametro mas de entrada)
  \tadAxioma{}{}

  ~

  \tadOperacion{ConjuntoCats}{Lista(categoria)}{conj(categoria)}{}
  \tadAxioma{}{}

  ~
 
  \AbsFc[estr]{abcat}{e.raiz = raiz(a) $\land$ ($\forall$ c:categoria)esta?(c,e) $\impluego$ id(e,c) = id(c,a) $\land$ ($\forall$ c:categoria)esta?(c,e) $\impluego$  }
...
La función ID debe tener complejidad O(|c|) donde c es la categoria de la cual se desea saber su id\\
La función padres debe tener complejidad O(|c|) donde c es la categoria de la cual se desea saber sus padres
...
La función Vacia debe tener complejidad O(1)\\
La función agregarAtras debe tener complejidad O(n) si y solo si se agregan n elementos de forma consecutiva\\
La función Lonitud debe tener complejidad O(1)\\
La función $\bullet$[$\bullet$] debe tener complejidad O(1)
