...
\begin{algorithmic}[1]
	\State $categoria \gets$ Obtener($res.aCategorias, c$)\Comment{O(|c|)}
	\State AgregarAtras$(res.linksPorCat[categoria.id], l)$\Comment{2O(1) = O(1)}
	\Statex
	\var{info}{infoLink}
	\State $info.categoria \gets c$\Comment{O(|c|)}
	\State $info.accesos \gets $Vacia()\Comment{O(1)}
	\State Definir$(res.infoLinks, l, info)$\Comment{O(|l|)}
\end{algorithmic}
