...
\Procedure{iNuevoAC}{\In{raiz}{categoria}}{ $\DRef res$ : \TipoVariable{abCat}}
	...
	\State $tupla.id \gets res.ultID$
	\State $tupla.hijas \gets $Vacia()
	\State $res.categorias\gets $Definir(Vacio(), $r$, $tupla$)\Comment{O(|$raiz$|)}
\EndProcedure
\end{algorithmic}
...
\Procedure{iAgregar}{\Inout{ac}{abCat}, \In{padre}{categoria}, \In{hija}{categoria}}
	\var{tupla}{Nodo}
	\var{hijo}{Nodo}
	\var{it}{itLista(categoria)}
	\var{list}{lista(categoria)}
	\State $ac.ultID \gets ac.ultID + 1$
	\State $tupla.id \gets ac.ultID$
	\State $tupla.hijas \gets $Vacia()
	\State Definir($ac.categorias$, $hija$, $tupla$)\Comment{O(|$hija$|)}
	\State $hijo \gets $significado($ac.categorias, padre$)\Comment{O(|$padre$|)}
	\State $list \gets hijo.hijas$
	\State $it \gets $agregarAtras($list, hija$)\Comment{O(|$hija$|)}
