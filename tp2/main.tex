...
1- Coloquialmente: El id de la raíz del árbol es 1 (requerido por el enunciado del TP).\newline
Formalmente: id(raiz) $=$ 1
	
2- Coloquialmente: No existe ninguna categoría incluida en sus categorías hijas.\newline
Formalmente: ($\forall$c:categoria)estaAbcat(c,e) $\Rightarrow$ (c $\neg\in$ hijos(c))

3- Coloquialmente: Todas las categorías tienen padre, excluyendo la raíz.\newline
Formalmente: ($\forall$c:categoria)(estaAbcat(c,e) $\yluego$ (c $\not$= raiz)) $\Rightarrow$ ($\exists$t:categoria)(estaAbcat(t,e) $\yluego$ t$\not$=c) $\yluego$ c $\in$ hijos(t)    

4- Coloquialmente: Existe una categoría que no tiene padre y que es igual a raíz.\newline
Formalmente: ($\exists$t:categoria)estaAbcat(t,e) $\yluego$ (($\neg\exists$c:categoria)estaAbcat(c,e) $\yluego$ $\Rightarrow$ raiz $\in$ hijos(c))

5- Coloquialmente: Cada categoría tiene un único padre (este ítem (junto con el 4) garantiza que el árbol no sea cíclico).\newline
Formalmente: ($\neg\exists$t,c:categoria)((estaAbcat(c,e)) $\yluego$ (estaAbcat(t,e)) (necesario???) $\yluego$ (t$\not$=c)) $\yluego$ ($\forall$k:categoria)estaAbcat(k,e) $\Rightarrow$ k $\in$ hijos(c) $\yluego$ k $\in$ hijos(t)

6- Coloquialmente: La cantidad de categorías totales es igual a UltId.\newline
Formalmente: 

7- Coloquialmente: Las categorías hijas de cada categoría tienen un id mayor a ésta.\newline
Formalmente:

8- Coloquialmente: No existen dos id's iguales para categorías distintas.\newline
Formalmente:

9- Coloquialmente: No existen dos categorías iguales en un mismo árbol.\newline
Formalmente:
