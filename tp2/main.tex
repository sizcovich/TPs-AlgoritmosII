...
\begin{Interfaz}
    ...
    \InterfazFuncion{padres}{\In{ac}{abCat}, \In{hija}{categoria}}{iteradorUniPadres}
	  [$\emph{est\'a}?(hija, ac)$]
...
\Rep[abcat][e]{$1 \land 2 \yluego 3 \land 4 \yluego 5 \yluego 6 \land 7 \land 8 \land 9$}\mbox{}
...
\begin{algorithm}[H]
\caption*{iPadres(\In{ac}{abCat}, \In{hija}{categoria}) $\DRef res$ : \TipoVariable{iteradorPadresUni(Nat)}}
\begin{algorithmic}[1]
    \State $nodoHija \gets \Call{Obtener}{ac.categorias, hija}$\Comment{O(|$hija$|)}
	\State $res \gets \Call{crearIteradorPadres}{nodoHija}$\Comment{O(1)}
\end{algorithmic}
\textbf{C\'alculo de complejidad}: $O(|hija|) + O(1) = O(|hija|)$
\end{algorithm}
